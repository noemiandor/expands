computeCellFrequencyDistributions<-function(dm, max_PM=6, precision, min_CellFreq=0.1, norm=TRUE){
  ...
  if(norm){
    print("Sum of cell frequency probability distributions will be scaled to 1");
  }else{
    print("Sum of cell frequency probability distributions will NOT be scaled to 1");
  }
  ...
  warnings=c();
  for (k in 1:nrow(dm)){
    output=try(cellfrequency_pdf(af=dm[k,"AF_Tumor"],cnv=dm[k,"CN_Estimate"], pnb=dm[k,"PN_B"],freq=freq, max_PM=max_PM),silent=TRUE);
